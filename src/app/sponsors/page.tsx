...
import { Building03Icon, UserGroupIcon, ChartLineData01Icon, SecurityCheckIcon } from 'hugeicons-react';
import { Medal, Users } from '@phosphor-icons/react';
  ...
  return (
    <PageContainer compact>
      ...
      <section className="mt-8 grid gap-3 md:grid-cols-4">
        <ImpactStatCard label="Athletes supported" value="128" icon={UserGroupIcon} />
        <ImpactStatCard label="Community reach" value="24k" icon={Users} tone="gold" />
        <ImpactStatCard label="Verified reports" value="426" icon={SecurityCheckIcon} tone="blue" />
        <ImpactStatCard label="Monthly growth" value="+38%" icon={ChartLineData01Icon} tone="orange" />
      </section>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {sponsorPackages.map((item) => (
            <div key={item.name} className="glass-panel flex flex-col rounded-xl p-5">
              <Building03Icon className="mb-5 size-6 text-[var(--goal-gold)]" />
            ...
            <button key={athlete.id} className="glass-panel rounded-xl p-4 text-left transition-all hover:-translate-y-1" onClick={() => router.push(`/athletes/${athlete.id}`)}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h3 className="mt-2 font-display text-lg font-black text-white">{athlete.name}</h3>
                </div>
              </div>
      ...
      <section className="mt-10 grid gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        <div>
          ...
          <div className="glass-panel rounded-xl p-5">
            ...
            <h3 className="font-display text-2xl font-black text-white">Annual Awards visibility</h3>
