...
import { Trophy, Medal, Star, Users } from '@phosphor-icons/react';
  ...
  return (
    <PageContainer compact>
      <section className="glass-panel relative overflow-hidden rounded-xl p-5 md:p-10">
        ...
        <div className="relative z-10 grid gap-8 lg:grid-cols-[1fr_0.85fr] lg:items-center">
          <div>
            ...
            <h1 className="font-display text-4xl font-black tracking-tight text-white md:text-6xl">GoalPlace Annual Awards</h1>
      ...
      <section className="mt-8 grid gap-3 md:grid-cols-4">
        ...
        <ImpactStatCard label="Verified moments" value="426" icon={SecurityCheckIcon} tone="orange" />
      </section>
          ...
          <div className="space-y-3">
            {leaders.map((leader, index) => (
              <div key={leader.name} className="glass-panel flex items-center gap-4 rounded-xl p-4">
                <div className="flex size-11 items-center justify-center rounded-xl border border-white/10 bg-white/8 font-display text-lg font-black text-[var(--goal-gold)]">
                  {index + 1}
                </div>
                ...
                <p className="font-display text-lg font-black text-[var(--goal-gold)]">{leader.points.toLocaleString()}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {awardCategories.slice(0, 12).map((category) => (
              <div key={category.id} className="glass-panel rounded-xl p-4">
                ...
                <h3 className="font-display text-base font-black text-white">{category.name}</h3>
              </div>
            ))}
          </div>
        </div>
          ...
          {athletes.slice(0, 6).map((athlete, index) => (
            <div key={athlete.id} className="glass-panel rounded-xl p-4">
              ...
              <div className="mt-4 rounded-lg border border-white/10 bg-white/5 p-3">
                ...
                <p className="mt-1 font-display text-lg font-black text-white">{formatUGX(athlete.totalEarnings ?? athlete.totalSupport ?? 0)}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {teams.slice(0, 6).map((team) => (
            <div key={team.id} className="glass-panel rounded-xl p-4">
              ...
              <h3 className="mt-4 font-display text-lg font-black text-white">{team.name}</h3>
