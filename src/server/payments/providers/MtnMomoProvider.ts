...
export class MtnMomoProvider implements PaymentProvider {
  ...
  async verifyCallback(request: Request): Promise<VerifiedProviderCallback | null> {
    ...
    const providerReference = intent.data()?.providerReference;
    if (!intent.exists || typeof providerReference !== 'string') return null;
    const status = await this.getCollectionStatus(providerReference);
    return {
      provider: this.name,
      eventId: `mtn:${payload.externalId}:${status.providerReference}:${status.status}`,
      paymentIntentId: payload.externalId,
      status: status.status === 'settled' ? 'settled' : status.status === 'failed' ? 'failed' : 'held_for_review',
      amountMinor: Number((status.raw as MtnStatusPayload).amount ?? payload.amount ?? 0),
      currency: 'UGX',
